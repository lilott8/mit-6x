...
class Patient(object):
    ...
    def update(self):
        ...
        # TODO
        #Step 1: doesclear? and do what it's supposed to
        virus = self.getViruses()
        for v in virus:
            if v.doesClear():
                self.viruses.remove(v)
        #step 2: update population density
        density = float(len(virus))/self.getMaxPop()
        virus = self.getViruses()[:]
        #Step 3: repopulate or not
        for v in virus:
            try:
                self.viruses.append(v.reproduce(self.getTotalPop()/float(self.maxPop)))
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    # TODO
    """
    init our necessary variables
    We need:
        -numViruses SimpleVirus
        -1 patient
    """
    viruses = []
    #number of steps to run through
    timeSteps = 300
    container = [0]*timeSteps
    
    for v in range(0, numViruses):
        viruses.append(SimpleVirus(maxBirthProb, clearProb))
    for trials in range(0, numTrials):
        patient = Patient(viruses, maxPop)
        #run the timesteps here
        for time in range(timeSteps):
            container[time] += float(patient.update())
    meanVirPop = [s/numTrials for s in container]
    
    pylab.plot(range(timeSteps), meanVirPop)
    pylab.title("28 Days Later")
    pylab.legend(('SimpleVirus',),loc='best')
    pylab.xlabel('Time Steps (s)')
